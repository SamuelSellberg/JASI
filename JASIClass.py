...
class JASI:
    def __init__(self,J,A,S,I,part=100,size=10):
    ...
    def arousal(self):
        return 1-self.I
    def valence(self,neg=None,raw=None):
        if raw==True:
            return self.J/(1-self.I)
        else:
            if neg==None:
                raise TypeError("valence() missing 1 required positional argument: 'neg'")
            if not isinstance(neg,(int,float)):
                raise TypeError("Negative barrier must be of type 'int' or 'float'.")
            if neg>1:
                raise ValueError("Negative barrier must be smaller than 1.")
            if neg<0:
                raise ValueError("Negative barrier must be larger than 0.")
            if self.I==1:
                return 0
            RAW=self.J/(1-self.I)
            if neg==1:
            ...
            for t in range(1,5):
                ...
                if t==i:
                    if self.I<emotion.I:
                        self.addI(1)
        return self
    def oppose(self,part=100,size=10):
        maximal=max(self.J,self.A,self.S,self.I)
        Jd=1-(self.J/maximal)
        Ad=1-(self.A/maximal)
        Sd=1-(self.S/maximal)
        Id=1-(self.I/maximal)
        maximal=max(Jd,Ad,Sd,Id)
        if maximal==0:
            return False
        k=1/maximal
        JD=Jd*k
        ...
        ID=Id*k
        x=1/(JD+AD+SD+ID)
        return JASI(JD*x,AD*x,SD*x,ID*x,part=part,size=size)
    def check(self,dic,prio=None):
        ...
        if prio==None:
            prio=max(priolist)
        if priolist.count(prio)==0:
            for i in range(prio+1):
                prio-=1
                if priolist.count(prio)>0:
                    break
                if prio<0:
                    raise Exception("No fields were found for this priority.")
        if priolist.count(prio)>1:
...
class AVI(JASI):
    def __init__(self,arousal,valence,importance,part=100,neg=.5,size=10):
        if not isinstance(arousal,(int,float)):
            raise TypeError("Arousal must be 'int' or 'float'.")
        if arousal<0 or arousal>1:
            raise ValueError("Arousal must be a value between 0 and 1.")
        if not isinstance(valence,(int,float)):
            raise TypeError("Valence must be 'int' or 'float'.")
        if valence<-1 or valence>1:
            raise ValueError("Valence must be a value between -1 and 1.")
        if not isinstance(importance,(int,float)):
            raise TypeError("Importance must be 'int' or 'float'.")
        if importance<-1 or importance>1:
            raise ValueError("Importance must be a value between -1 and 1.")
        if not isinstance(part,(int,float)):
            raise TypeError("'part' must be 'int' or 'float'.")
        if part<0:
        ...
        self.nS=(1-self.S)*self.__size__
        self.nI=(1-self.I)*self.__size__
        self.__nochange__=(arousal,valence,importance)
    def jasi(self):
        return 'J = %s, A = %s, S = %s, I = %s' %(self.J,self.A,self.S,self.I)
    def makeJASI(self):
        return JASI(self.J,self.A,self.S,self.I,self.part,self.__size__)
    def __repr__(self):
        arousal=self.arousal()
        valence=self.valence(self.neg)
        importance=self.importance()
        if allclose(self.__nochange__[0],arousal) and allclose(self.__nochange__[1],valence) and allclose(self.__nochange__[2],importance):
            arousal=self.__nochange__[0]
            valence=self.__nochange__[1]
            importance=self.__nochange__[2]
        return 'A = %s, V = %s, I = %s' %(arousal,valence,importance)
...
def randadd(startJASI,times,bound=1,siderot=-60,toprot=30,slide=-0.45,straightup=False):
    if not isinstance(startJASI,JASI):
        raise TypeError("Function need a start JASI-value.")
    if not isinstance(times,int):
        raise TypeError("times must be of type integer.")
    if not isinstance(bound,int):
        raise TypeError("bound must be of type integer.")
    if not isinstance(siderot,(int,float)) or not isinstance(toprot,(int,float)):
        raise TypeError("Degrees must be 'int' or 'float'.")
    if not isinstance(slide,(int,float)):
        raise TypeError("Slide must be 'int' or 'float'.")
    ...
    ax.plot3D((i,i),(j,j),(d,d),marker='o',color="white")
    ax.text(i,j,d-0.25,"I",color='gray')
    x=startJASI.J*b+startJASI.A*c+startJASI.S+startJASI.I*i
    y=-startJASI.J+startJASI.A+startJASI.S*a+startJASI.I*j
    z=1-startJASI.I*(L*sqrt(2/3))
    ax.plot3D((x,x),(y,y),(z,z),marker='.',color="black")
    for G in range(times):
        tim=random.randint(1,bound)
        #random.choice((startJASI.addJ(tim),startJASI.addA(tim),startJASI.addS(tim),startJASI.addI(tim)))
            ...
            if used.get(j)!=i:
                ...
                if i!=j:
                    if keys[i].prio==keys[j].prio:
                        if keys[i].J>keys[j].j and keys[j].J>keys[i].j:
                            problems.append("Keys %s and %s with the same priority occupy the same J-values" %(i,j))
                        if keys[i].A>keys[j].a and keys[j].A>keys[i].a:
                            problems.append("Keys %s and %s with the same priority occupy the same A-values" %(i,j))
                        if keys[i].S>keys[j].s and keys[j].S>keys[i].s:
                            problems.append("Keys %s and %s with the same priority occupy the same S-values" %(i,j))
                        if keys[i].I>keys[j].i and keys[j].I>keys[i].i:
                            problems.append("Keys %s and %s with the same priority occupy the same I-values" %(i,j))
    if problems==[]:
        return "No problems where found."
    else:
        return print(str(problems).strip("[]").replace("'",""))
...
show=JASI(.4,.2,.05,.35)
avi=AVI(.6,-0.1,.4)
joy=JASIfield(0.2,1,0,0.8,0,0.8,0,0.8,prio=0,colour="greenyellow")
tester=JASIfield(0.25,0.5,0,0.25,0,0.25,0,1,prio=1,colour="turquoise")
emdic={joy: 'Joy',tester: 'Test'}